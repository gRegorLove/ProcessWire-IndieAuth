...
/** @internal */
final class TestSuiteParser extends \IndieAuth\Libs\Mf2\Parser
{
    ...
    public function textContent(\DOMElement $el, $dummy = \false)
    {
        ...
        return \IndieAuth\Libs\Mf2\unicodeTrim($clonedEl->textContent);
    }
    ...
    private function _resolveChildUrls(\DOMElement $element)
    {
        if (null === $this->__resolveChildUrls) {
            ...
            $this->__resolveChildUrls->setAccessible(\true);
        }
        return $this->__resolveChildUrls->invoke($this, $element);
    }
}
/** @internal */
class MicroformatsTestSuiteTest extends \IndieAuth\Libs\PHPUnit_Framework_TestCase
{
    /**
    ...
    public function testMf1FromTestSuite($input, $expectedOutput)
    {
        ...
        $this->assertEquals($this->makeComparible(\json_decode($expectedOutput, \true)), $this->makeComparible(\json_decode(\json_encode($parser->parse()), \true)));
    }
    /**
    ...
    public function htmlAndJsonProvider($subFolder = '')
    {
        ...
        $finder = new \RegexIterator(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator(\dirname(__FILE__) . '/../../vendor/' . $subFolder, \RecursiveDirectoryIterator::SKIP_DOTS)), '/^.+\\.html$/i', \RecursiveRegexIterator::GET_MATCH);
        ...
        foreach ($finder as $key => $value) {
            ...
            if (\is_file($dir . '/' . $result)) {
                $tests[$testname] = array('input' => \file_get_contents($dir . '/' . $test), 'expectedOutput' => \file_get_contents($dir . '/' . $result), 'name' => $testname);
            }
        }
        return $tests;
    }
